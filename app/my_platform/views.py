...
from .forms import (
    Embed_Ownership_Image_Form,
    Embed_Enforcement_Sound_Form,
    Embed_Enforcement_Text_Form,
    Embed_Enforcement_Image_Form,
    Embed_Ownership_Sound_Form,
    Embed_Ownership_Text_Form,
    CreateUserForm,
    UserForm,
    Extract_Embedded_Text_Form,
    Extract_Embedded_Sound_Form,
    Extract_Embedded_Image_Form,
)
from .models import (
    Embed_Ownership_Text,
    Embed_Ownership_Image,
    Embed_Enforcement_Sound,
    Embed_Enforcement_Text,
    Embed_Enforcement_Image,
    Embed_Ownership_Sound,
    User_Info,
    Embedded_Files,
    Extract_Embedded_Text,
    Extract_Embedded_Sound,
...
def account_profile(request):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            "Your profile information was successfully updated " + current_user.name,
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                pdf_embed_watermark(
                    last_object_location, watermarkInfo, last_object_location
                )
                ...
                messages.success(
                    ...
                    + ". Go to Embedded Files to download document. ",
                )
                return render(
                    request,
                    "embed_ownership_text.html",
                    {
                ...
                messages.error(
                    request,
                    "FileType Error: File embedding failed. Hash of original file stored as record for "
                    + current_user_info.name
                    + ". Go to Embedded Files to download document. ",
                )
                return render(
                    ...
                    "embed_ownership_text.html",
                    {
                        "last_object": last_object,
                        "form": form,
                        "watermarkInfo": last_object.hash,
                    },
                )

    else:
        #
        form = Embed_Ownership_Image_Form(initial={"user_info": current_user_info})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(
                    request,
                    "embed_ownership_sound.html",
                    {"last_object": last_object, "form": form},
                )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(
                    request,
                    "File embedding failed. Hash of original file stored as record for "
                    + current_user_info.name
                    + ". Go to Embedded Files to download document. ",
                )
                return render(
        ...
        form = Embed_Enforcement_Text_Form(initial={"user_info": current_user_info})
        form.fields["user_info"]._queryset = current_user_info
...
def embed_enforcement_image(request):
    ...
    if request.method == "POST":
        ...
        form = Embed_Enforcement_Image_Form(initial={"user_info": current_user_info})
        form.fields["user_info"]._queryset = current_user_info
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(
                    request,
                    "embed_enforcement_sound.html",
                    {"last_object": last_object, "form": form},
                )

            except:
                return HttpResponse(
                    "File failure to upload, please check your internet connection"
                )
...
def embedded_files(request):
    if request.user.is_authenticated:
        try:
            ...
            uploaded_files = []
            return render(
                request, "embedded_files.html", {"uploaded_files": uploaded_files}
            )
    else:
        messages.info(request, "Please login or create an account")
        return render(request, "login.html")
...
def extract_text_info(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            last_object_location = MEDIA_ROOT + f"{last_object.file}"
            last_object.uploaded_filename = str(last_object.file).split("/")[-1]
            last_object.hash = get_hash(last_object_location)
            hash = (last_object.hash,)
            ...
            try:
                ...
                if queryresult == []:
                    sql_enforcement = (
                        " SELECT * FROM digital_enforcement_tbl WHERE hash = %s"
                    )
                ...
                messages.success(
                    request,
                    "Information extracted for file "
                    + last_object.uploaded_filename
                    + ". Verify owner below. ",
                )
                return render(
                    request,
                    "extract_text_info.html",
                    {"last_object": last_object, "form": form},
                )
                ...
                messages.error(
                    request,
                    "File embedding was not found for "
                    + f"{last_object.file}"
                    + ". Go to Embedded Files to download document. ",
                )
                return render(
